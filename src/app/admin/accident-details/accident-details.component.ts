...
export class AccidentDetailsComponent implements OnInit {
  ...
  getHealthWhere(id: any): void {
    ...
    this.http.get(environment.baseUrl + 'auth/accident-where/' + id, {headers: header})
      .subscribe((res: any) => {
          this.accidents = res.data;
          this.studentObj = res.student;
          console.log('student', this.accidents);
        },
        error => {
          Swal.fire(
            'The Internet?',
  ...
  delete(id: any): void {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.http.delete(environment.baseUrl + 'auth/accident/' + id)
          .subscribe((res: any) => {
              console.log(this.id);
